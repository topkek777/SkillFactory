...
def convert_result_to_json(res):
    """Функция преобразовывает ответ в формат JSON, если это возможно.
    В противном случае - в текстовый формат."""
    try:
        result = res.json()
...
class PetFriends:
    """API-библиотека для веб-приложения PetFriends"""
    ...
    def get_api_key(self, email: str, passwd: str) -> json:
        ...
        JSON с уникальным ключем пользователя, найденным по указанным email и паролем"""
        ...
        result = convert_result_to_json(res)
    ...
    def add_new_pet_without_photo(self, auth_key: json, name: str, animal_type: str, age: str):
        """Метод отправляет (постит) на сервер данные о добавляемом питомце (без фото) и возвращает
    ...
    def add_pet_photo(self, auth_key: json, pet_id: str, pet_photo: str):
        """Метод отправляет на сервер запрос на добавление фото питомца по указанному ID
        и возвращает статус запроса и результат в формате JSON с данными обновленного питомца"""
